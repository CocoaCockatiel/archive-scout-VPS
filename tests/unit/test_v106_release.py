...
class V106ReleaseTests(unittest.TestCase):
    def test_release_identity(self):
        self.assertEqual(VERSION, '1.0.6.5')
    ...
    def test_windows_file_version_metadata_matches_release(self):
        ...
        self.assertIn('filevers=(1, 0, 6, 5)', metadata)
        self.assertIn('prodvers=(1, 0, 6, 5)', metadata)
        self.assertIn("StringStruct('FileVersion', '1.0.6.5')", metadata)
        self.assertIn("StringStruct('ProductVersion', '1.0.6.5')", metadata)
